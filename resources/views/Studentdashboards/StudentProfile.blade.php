              ...
              <div class="card card-primary card-outline shadow-lg p-3 mb-3 bg-white" style="border-radius: 6px;">
                <div class="card-body box-profile">
                  <div class="text-center">
                    <img class="profile-user-img img-fluid img-circle student_picture" src="{{ Auth::user()->picture }}" alt="User profile picture">
                  </div>
  
                  <h3 class="profile-username text-center student_name" >{{Auth::user()->name}}</h3>
  
                  <p class="text-center">Student</p>

                  <input type="file" name="student_image" id="student_image" style="opacity: 0;height:1px;display:none">
                ...
                <div class="card-body">
                  <div class="tab-content">
                    <div class="active tab-pane" id="personal_info">
                      <form class="form-horizontal" method="POST" action="{{ route('studentUpdateInfo') }}" id="StudentInfoForm">
                        ...
                        <div class="form-group row">
                          ...
                          <div class="col-sm-10">
                            <input type="text" class="form-control" id="inputStudentID" placeholder="Student ID" value="{{ Auth::user()->studentId }}" name="studentId">
                            <span class="text-danger error-text studentId_error"></span>
                          </div>
                        </div>
                    ...
                    <div class="tab-pane" id="change_password">
                        <form class="form-horizontal" action="{{ route('studentChangePassword') }}" method="POST" id="changePasswordStudentForm">
                          <div class="form-group row">
 ...
 <!-- Profile -->
    ...
    <script>
      ...
      $(function(){
              ...
              success:function(data){
                    if(data.status == 0){
                      ...
                      $('.student_name').each(function(){
                        $(this).html( $('#StudentInfoForm').find( $('input[name="name"]') ).val() );
                      });
                      alert(data.msg);
                    }
              }
        ...
        $('#changePasswordStudentForm').on('submit', function(e){
            e.preventDefault();
            ...
            $.ajax({
                ...
                success:function(data){
                  if(data.status == 0){
                    ...
                    $('#changePasswordStudentForm')[0].reset();
                    alert(data.msg);
                  }
                }
            });
